...
var StandardCalculator = require  ("./StandardCalculator")
...
module.exports = kind({
	  ...
	  onMemoryActiveChanged: "memoryActiveChanged" },
	{
	    ...
	    components: [
		{ kind: Button, name: "theButton", content: "Start", ontap: "tapped" },
		{
		    name: "resultsSummary",
		    content: "Tests",
	...
	{
	    name: "testRows", kind: Scroller, fit: true,
	...
	if (this.currentTest < this.tests.length) {
	    ...
	    logger.log("Finished testing. Fail " + this.nTestsFailed + " of " + this.nTestsRun);
	    clearInterval(this.job);
    ...
    tests: [
	{ desc: ("Initial state"), keys: [], expect: "0" }, // Being the first test is very much implied!      ///{ desc: $L("Initial state"), keys: [], expect: "0" }, // Being the first test is very much implied!
	{ desc: "=", keys: ["equals"], expect: "0" },
	{ desc: ("Clear"), keys: ["clear"], expect: "0" },			//{ desc: $L("Clear"), keys: ["clear"], expect: "0" },
